...
@section('content')
    ...
    <p class="drive-notice">
        ...
        It is served from Google Drive, so the “bro tour” folder has to be shared as
        <em>Anyone with the link — Viewer</em>. <a href="https://drive.google.com/drive/folders/1xbeKOA-j7HGA3AnxhSKZrFVb2YovoH-Y" target="_blank" rel="noopener">Open the folder</a>
        and check the sharing setting, or copy it into <code>public/img/tours</code> and set
        <code>DRIVE_MODE=local</code>.
    </p>
    ...
    <section class="filters">
        <div class="wrap filters__inner">
            ...
            <ul class="filters__list">
                <li><a class="{{ !$active ? 'is-active' : '' }}" href="{{ url('/tours') }}">All {{ count(\App\Support\Tours::all()) }}</a></li>
    ...
    <section class="itn itn--page tours-grid">
        ...
        @if (!$tours)
            <div class="wrap"><p class="prose">Nothing in that group yet — <a href="{{ url('/tours') }}">see all twenty trips</a> instead.</p></div>
        @endif
    </section>
        ...
        <div class="wrap practical__grid">
            <div class="practical__text">
                ...
                <p class="prose reveal-up" data-reveal>
                    Trip prices are per person and shown when you ask, because they move with the season, the boat and how many
                    of you there are. Private versions of almost everything here cost more per head and are worth it on the
                    boat days in particular.
                </p>
                <p class="prose reveal-up" data-reveal>
                    Pick-up is from your hotel gate, not the lobby, and the driver will phone the reception desk if the gate is
                    locked. Cancellations are free until 24 hours before the start time — the boats and the quads are held for
                    you overnight.
                </p>
            </div>
            <ul class="practical__list reveal-up" data-reveal>
                ...
                <li><span>What to bring</span><em>Swimwear, towel, sunscreen, shoes that can get wet, a card for tips.</em></li>
